...
export function CoverageDetailScreen() {
  ...
  const COVERED_ROWS = [
    { label: language === 'en' ? 'Room & board / day' : 'ค่าห้องและค่าอาหาร / วัน', value: '฿8,000' },
    { label: language === 'en' ? 'ICU / day' : 'ค่าห้อง ICU / วัน', value: '฿16,000' },
    { label: language === 'en' ? 'Surgery & procedures' : 'ค่าผ่าตัดและหัตถการ', value: language === 'en' ? 'Actual cost' : 'ตามจริง' },
    { label: language === 'en' ? 'Physician fee / day' : 'ค่าแพทย์ตรวจรักษา / วัน', value: '฿2,000' },
  ];

  const NOT_COVERED = language === 'en'
    ? ['Pre-existing conditions', 'Cosmetic treatments', 'Drugs or services not medically necessary']
    : ['โรคที่เป็นมาก่อนทำประกัน', 'การรักษาความงาม', 'ยาหรือบริการที่ไม่จำเป็นทางการแพทย์'];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.screenBg }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: screenPadding, paddingTop: 12, paddingBottom: 16, gap: 8 }}>
        ...
        <Text style={{ fontFamily: fontFamily.anuphan.bold, fontSize: 17, color: colors.ink }}>{s.policy.coverageDetailTitle}</Text>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}
        ...
        <View style={{ backgroundColor: colors.card, borderRadius: radius.card, padding: 20, gap: 8, ...cardShadow }}>
          <Text style={{ fontFamily: fontFamily.anuphan.medium, fontSize: 12, color: colors.textSecondary }}>{s.policy.annualLimit}</Text>
          <Text style={{ fontFamily: fontFamily.jakarta.extraBold, fontSize: 36, color: colors.ink, letterSpacing: -1 }}>{language === 'en' ? 'Full' : 'เต็มจำนวน'}</Text>
          <View style={{ gap: 6, marginTop: 4 }}>
            ...
            <Text style={{ fontFamily: fontFamily.anuphan.regular, fontSize: 12, color: colors.textSecondary }}>{language === 'en' ? 'Used this year ฿38,500' : 'ใช้แล้วปีนี้ ฿38,500'}</Text>
          </View>
          <View style={{ backgroundColor: colors.successTint, borderRadius: 10, padding: 10, flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 }}>
            ...
            <Text style={{ flex: 1, fontFamily: fontFamily.anuphan.regular, fontSize: 12, color: colors.successDeep, lineHeight: 18 }}>
              {language === 'en' ? 'Cashless treatment at 400 partner hospitals' : 'เข้ารักษาในโรงพยาบาลคู่สัญญา 400 แห่งโดยไม่ต้องสำรองจ่าย'}
            </Text>
          </View>
        </View>
        ...
        <View style={{ backgroundColor: colors.card, borderRadius: radius.card, overflow: 'hidden', ...cardShadow }}>
          <View style={{ paddingHorizontal: 16, paddingTop: 14, paddingBottom: 8 }}>
            <Text style={{ fontFamily: fontFamily.jakarta.bold, fontSize: 11, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8 }}>{s.policy.covered}</Text>
          </View>
        ...
        <View style={{ backgroundColor: colors.card, borderRadius: radius.card, overflow: 'hidden', ...cardShadow }}>
          <View style={{ paddingHorizontal: 16, paddingTop: 14, paddingBottom: 8 }}>
            <Text style={{ fontFamily: fontFamily.jakarta.bold, fontSize: 11, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8 }}>{s.policy.notCovered}</Text>
          </View>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: colors.screenBg, paddingHorizontal: screenPadding, paddingTop: 12, paddingBottom: insets.bottom + 16, borderTopWidth: 1, borderTopColor: colors.hairline }}>
        <TouchableOpacity activeOpacity={0.82}
          ...
          <Text style={{ color: colors.white, fontFamily: fontFamily.anuphan.bold, fontSize: 16 }}>{s.policy.claimBtn}</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
